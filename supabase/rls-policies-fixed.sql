...
-- Helper function to get current user's ID from JWT (using public schema)
...
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    ...
    WHERE org_id = org_uuid AND user_id = public.current_user_id()
  );
$$ LANGUAGE sql STABLE SECURITY DEFINER;

-- Helper function to check if user is project member
CREATE OR REPLACE FUNCTION public.is_project_member(proj_uuid UUID)
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1 FROM project_members pm
    JOIN projects p ON p.id = pm.project_id
    WHERE pm.project_id = proj_uuid
    AND (pm.user_id = public.current_user_id() OR p.owner_id = public.current_user_id())
  );
$$ LANGUAGE sql STABLE SECURITY DEFINER;

-- Helper function to check if user has access to project through org membership
CREATE OR REPLACE FUNCTION public.has_project_access(proj_uuid UUID)
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1 FROM projects p
    JOIN org_members om ON om.org_id = p.org_id
    WHERE p.id = proj_uuid AND om.user_id = public.current_user_id()
  );
$$ LANGUAGE sql STABLE SECURITY DEFINER;
...
CREATE POLICY "Users can view profiles in their organizations"
  ...
  USING (
    EXISTS (
      ...
      WHERE om1.user_id = public.current_user_id() AND om2.user_id = users.id
    )
  );
...
CREATE POLICY "Users can insert projects in their organizations"
  ...
  WITH CHECK (public.is_org_member(org_id) AND owner_id = public.current_user_id());

CREATE POLICY "Users can update projects they own or are members of"
  ON projects FOR UPDATE
  USING (owner_id = public.current_user_id() OR public.is_project_member(id));
...
CREATE POLICY "Project owners can add members"
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE id = project_id AND owner_id = public.current_user_id()
    )
  );
...
CREATE POLICY "Users can view documents in accessible projects"
  ...
  USING (public.has_project_access(project_id) OR public.is_project_member(project_id));

CREATE POLICY "Users can insert documents in accessible projects"
  ON documents FOR INSERT
  WITH CHECK (
    (public.has_project_access(project_id) OR public.is_project_member(project_id))
    AND uploaded_by = public.current_user_id()
  );

CREATE POLICY "Users can update their own documents"
  ON documents FOR UPDATE
  USING (uploaded_by = public.current_user_id() OR public.is_project_member(project_id));
